  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { name, email, age, sex, address, medicalHistory } = response.data;

        setPatient({ name, email, age, sex, address });
        //console.log(name)

        console.log("Processed medical history:", medicalHistory);  //simple problem of not displayiig the patient.name


        if (medicalHistory.length > 0) {
          setMedicalHistories(medicalHistory);
        }
